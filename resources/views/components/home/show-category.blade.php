        ...
        <div class="items-center">
            <div>
                <form action="{{ route('filter.products') }}" method="POST">
                    @csrf
                    @method('POST')
                <button type="submit" class="wow fadeInUp" data-wow-delay="0.2s" href=""><img class="transition-all delay-75 hover:scale-[1.2]" src="{{ URL::to('storage/'.$item->image_url) }}" alt=""></button>
                </form>
                </div>
            
            <div><p class="text-center">{!! session('lang') == 'en' ? $item->name_en : $item->name_ar !!}</p></div>
        </div>  
        @endif
