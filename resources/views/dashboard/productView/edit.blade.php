<x-app-layout>
    ...
    <div class="text-2xl py-4 font-bold underline text-gray-900 d:text-white">Product Views</div>
    <!-- Breadcrumb -->
    ...
    <form class="pt-4" action="{{ route('productView.update',$record->id) }}" enctype="multipart/form-data" method="POST">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-1">
            <div class="grid grid-cols-1 gap-4 border-2 p-4 md:grid-cols-2">
                <div>
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 d:text-white">Name
                        English</label>
                ...
                <div>
                    ...
                    <select id="collection_id" name="collection_id"
                        ...
                        @foreach ($collections as $collection)
                            <option value="{{$collection->id}}" @if($collection->id == $record->collection_id) selected @endif>{{$collection->name_en}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                {{-- <div>
                    ...
                    <input type="checkbox" id="selected" @if ($record->selected == 1) @checked(true) @endif  value="{{$record->selected}}" name="selected"
