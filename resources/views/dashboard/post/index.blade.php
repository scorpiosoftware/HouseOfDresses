<x-app-layout>
    ...
    @endif
    <div class="text-2xl py-4 font-bold underline text-gray-900 dark:text-white">Posts</div>
    <div class="relative m-4 md:flex justify-between items-center">
        ...
        <form action="{{ route('post.index', 'search') }}" class="w-full max-w-md " method="GET">
            @csrf
            @method('GET')
            ...
            <div class="relative">
                ...
                <input type="search" id="search" name="search"
                    ...
                    placeholder="Search by post url ..." />
                <button type="submit"
            ...
            <tbody>
                @foreach ($records as $record)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <th scope="row" class="px-6 text-gray-900  dark:text-white">
                            <img src="{{ URL::to('storage/' . $record->image_url) }}" alt=""
                                class="rounded-md w-24 h-24 shadow-lg ">
                        </th>

                        <td class="px-6 py-4 font-extrabold whitespace-nowrap text-black text-base">
                            {{ $record->post_url }}
                        </td>
                        <td class="px-6 py-3 text-right flex justify-start items-center pt-10 space-x-4">
                            <a href="{{ route('post.edit', $record->id) }}"
