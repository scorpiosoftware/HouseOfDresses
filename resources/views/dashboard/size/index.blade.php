<x-app-layout>
    ...
    @endif
    <div class="text-2xl py-4 font-bold underline  text-[#b69357] dark:text-white">Sizes</div>
    <div class="relative m-4 md:flex justify-between items-center">
        <a href="{{ route('size.create') }}"> <button type="button"
        ...
        <form action="{{ route('size.index', 'search') }}" class="w-full max-w-md " method="GET">
            @csrf
            @method('GET')
                ...
                @foreach ($records as $record)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-3 text-right flex justify-start items-center pt-[30px] space-x-4">
                            ...
                            <form action="{{ route('size.destroy', $record->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
