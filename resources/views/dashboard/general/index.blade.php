<x-app-layout>
    ...
    @endif
    <div class="text-2xl py-4 font-bold underline text-gray-900 d:text-white">General Info</div>
    <div class="relative m-4 md:flex justify-between items-center">
        <form action="{{ route('general.index', 'search') }}" class="w-full max-w-md " method="GET">
            @csrf
            @method('GET')
            ...
            <tbody>
                @foreach ($records as $record)
                    <tr class="bg-white border-b d:bg-gray-800 d:border-gray-700">
                        ...
                        <td class="px-6 py-4 truncate text-black text-base overflow-x-hidden text-nowrap">
                            {{-- {{ $record->shipping }} --}}
                        </td>
                        ...
                        <td class="px-6 py-3 text-right flex justify-start items-center pt-[30px] space-x-4 text-nowrap">
                            <a href="{{ route('general.edit', $record->id) }}"
