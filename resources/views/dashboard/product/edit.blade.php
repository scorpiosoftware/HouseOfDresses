        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <a href="/dashboard/color/create?product={{$record->id}}" class="ms-1 text-sm font-medium text-gray-500 md:ms-2 d:text-gray-400">add color</a>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <form class="pt-4" action="{{ route('product.update', $record->id) }}" enctype="multipart/form-data"
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-1">
            <div class="grid grid-cols-1 gap-4 border-2 p-4 md:grid-cols-2">
                ...
                <div>
                    <label for="collection_id"
                        class="block mb-2 text-sm font-medium text-gray-900 d:text-white">Collection</label>
                    <select id="collection_id" name="collection_id"
                        class="bg-gray-50 border border-gray-300 text-gray-900 mb-6 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 d:bg-gray-700 d:border-gray-600 d:placeholder-gray-400 d:text-white d:focus:ring-blue-500 d:focus:border-blue-500">
                        @foreach ($collections as $collection)
                            <option value="{{ $collection->id }}" @if ($record->collection_id == $collection->id) selected @endif>
                                {{ $collection->name_en }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="w-full">
                    <label for="sizes"
                        class="block mb-2 text-sm font-medium text-gray-900 d:text-white">Sizes</label>
                    <div class="relative flex w-full">
                        <select id="sizes" name="sizes[]" multiple placeholder="Select Size..."
                            autocomplete="off" class="block w-full rounded-sm cursor-pointer focus:outline-none"
                             required>
                            @foreach ($sizes as $size)
                                <option value="{{ $size->id }}"
                                    @if ($record->sizes()->Where('id', '=', $size->id)->find($size->id)) selected @endif>{{ $size->name }}
                                </option>
                            @endforeach
            ...
            <div class="grid grid-cols-1 gap-4 border-2 p-4 md:grid-cols-2">
                <div>
                    <label for="price"
                        class="block mb-2 text-sm font-medium text-gray-900  d:text-white">USD PRICE</label>

                    <input type="number" id="price" value="{{ !empty($record->price) ? $record->price : 0.0 }}" name="price"
                        step=".01"
                ...
                <div>
                    <label for="price2"
                        class="block mb-2 text-sm font-medium text-gray-900  d:text-white">ADE PRICE</label>

                    <input type="number" id="price2" value="{{ !empty($record->price2) ? $record->price2 : 0.0 }}" name="price2"
                        step=".01"
                        class="bg-gray-50 h-12 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 d:bg-gray-700 d:border-gray-600 d:placeholder-gray-400 d:text-white d:focus:ring-blue-500 d:focus:border-blue-500"
                        placeholder="" required />
                </div>
