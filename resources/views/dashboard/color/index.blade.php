<x-app-layout>
    ...
    @endif
    <div class="text-2xl py-4 font-bold underline text-[#b69357] d:text-white">Product Colors</div>
    <div class="relative m-4 md:flex md:justify-between md:items-center">
        <div class="md:flex grid grid-cols-1">
            <a href="{{ route('color.create') }}"> <button type="button"
                    class="focus:outline-none shadow-lg text-[#b69357] bg-white border-[#b69357] hover:bg-green-200 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 d:bg-green-600 d:hover:bg-green-700 d:focus:ring-green-800">
                    Create</button></a>
            <div>
                <form action="{{ route('color.index', 'search') }}" class="w-full max-w-md " method="GET">
                    <select id="product" onchange="this.form.submit()" name="product" 
                        ...
                        <option value="{{ $product->id }}"@if (!empty($inputs['product']) and $inputs['product'] ==  $product->id) selected @endif>{{ $product->name_en }}</option>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 d:text-gray-400">
            ...
            <tbody>
                @foreach ($records as $record)
                    <tr class="bg-white border-b text-start d:bg-gray-800 d:border-gray-700">
                        ...
                        <td class="px-6 py-4 text-base text-nowrap">
                             <span class="rounded-full px-5 py-2 border" style="background-color: {!! $record->color !!}"></span>
                        </td>
                        <td class="px-6 py-6 text-center flex justify-start items-center  space-x-4">
